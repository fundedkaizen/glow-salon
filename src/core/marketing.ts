...
const RUNS: Record<string, { perDay: number[]; blurb: string; permanent?: boolean }> = {
  flyers: { perDay: [2, 2, 2], blurb: '+2 customers a day for 3 days, mostly neighbours: grandmas, students, teachers.' },
  social: { perDay: [3, 3, 3, 3, 3], blurb: '+3 young customers a day for 5 days.' },
  radio: { perDay: [4, 4, 4], blurb: '+4 customers a day for 3 days: office workers, mechanics, chefs, nurses.' },
  magazine: { perDay: [2, 2, 2, 2, 2, 2, 2], blurb: '+2 customers a day for a week, the kind who love a treat: models, lawyers, brides.' },
  loyalty: { perDay: [1], permanent: true, blurb: '+1 customer every day, for good. Your regulars pop in more often.' },
  influencer: { perDay: [1, 3, 3], blurb: 'An influencer visits tomorrow, then +3 customers a day for 2 days from the video.' },
}

export const CAMPAIGNS: CampaignDef[] = WORLD_DATA.marketing.map(m => {
  const run = RUNS[m.id] ?? { perDay: [1], blurb: m.effect }
  return { id: m.id, label: m.label, price: m.cost * CAMPAIGN_UNIT, brings: [...m.brings], perDay: run.perDay, permanent: !!run.permanent, blurb: run.blurb }
})
...
export function canRunCampaign(id: string, active: readonly ActiveCampaign[], loyalty: boolean, money: number): CampaignCheck {
