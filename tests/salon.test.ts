...
export function run() {
  ...
  check('every item has a price and tab', ITEMS.every(i => i.price > 0 && i.tab))
  check('something affordable after day 1 (< $150)', ITEMS.filter(i => !i.soon && !i.needs && i.price <= 150).length >= 3)
  check('base customers', customersPerDay([]) === 3 && customersPerDay(['flyers']) === 4)
  ...
  check('day 1 brings Mira first', plans[0].regular === 'mira')
  check('arrivals ordered', plans.every((p, i) => i === 0 || p.arriveAt > plans[i - 1].arriveAt))
  check('no disaster day 1', plans.every(p => !p.disaster))
  ...
  check('schedule planned', state.schedule.length === 3)
  ...
  check('receipt adds up', rc.net === rc.revenue + rc.tips - rc.costs && rc.served === 3, rc)
  check('receipt rating', rc.ratingAfter > 0 && rc.reviews.length === 3)
  check('awards', awards(state.stats).length >= 2)
  check('mira is now met', state.met.includes('mira'))
  ...
  check('nobody walks out', waitState.customers.length === 3 && waitState.phase === 'closing')
  ...
  check('stations kept', state.stations.length === 2 && state.players.length === 1)
